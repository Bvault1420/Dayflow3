  ...
  if (!user) {
    return (
      <div className="flex h-full flex-col items-center justify-center px-6 text-center">
        <p className="font-display text-2xl font-bold text-white">Your profile awaits</p>
        <p className="mt-2 text-sm text-white/50">Sign in to see creations, likes, and saves.</p>
        <button
          type="button"
          onClick={onOpenAuth}
          className="mt-6 rounded-2xl bg-aippy-green px-6 py-3 text-sm font-bold text-black"
        >
          Log in
        </button>
      </div>
  ...
  return (
    <div className="h-full overflow-y-auto pb-28 scrollbar-hide">
      <div className="flex items-center justify-between px-4 pb-2 pt-12">
        <div className="flex items-center gap-2 rounded-full bg-white/5 px-3 py-1.5">
          ...
          <div className="h-1.5 w-16 overflow-hidden rounded-full bg-white/15">
            <div className="h-full w-2/5 rounded-full bg-aippy-green" />
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 rounded-full bg-white/5 px-2.5 py-1.5 text-sm font-semibold text-aippy-green">
            <Gem className="h-3.5 w-3.5" />
            {profile?.coins ?? 100}
          </div>
          <button type="button" className="text-white/70" aria-label="Settings">
            <Settings className="h-5 w-5" />
          </button>
          <button type="button" onClick={() => signOut()} className="text-white/70" aria-label="Log out">
      ...
      <div className="mt-4 flex flex-col items-center px-4 text-center">
        <div className="flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-fuchsia-500 via-violet-500 to-cyan-400 text-3xl font-bold text-white shadow-lg">
          {name.slice(0, 1).toUpperCase()}
        </div>
        <h1 className="mt-3 font-display text-2xl font-bold text-white">{name}</h1>
        <button
          type="button"
        ...
        <p className="mt-2 text-sm text-white/40">{profile?.bio || "No bio yet..."}</p>

        <div className="mt-4 flex w-full max-w-sm gap-2">
          <button
            type="button"
            className="flex-1 rounded-full bg-white/10 py-2.5 text-sm font-semibold text-white"
          >
            Edit Profile
          </button>
          <button
      ...
      <div className="mt-6 flex items-center justify-around border-b border-white/10 px-6">
        {(
          [
            ["created", Grid2x2],
            ["liked", Heart],
      ...
      <div className="grid grid-cols-2 gap-3 p-3">
        {tab === "created" && (
          <div className="flex aspect-[3/4] flex-col justify-between rounded-2xl bg-[#0f3d2e] p-3">
            <span className="w-fit rounded-full bg-white/15 px-2 py-0.5 text-[10px] font-semibold text-white">
              Draft: {drafts.length}
            </span>
            <div>
              <p className="font-display text-lg font-bold text-white">Your Draft</p>
              <p className="text-xs text-white/60">Build What You Feel.</p>
        ...
        {loading && (
          <p className="col-span-2 py-10 text-center text-sm text-white/40">Loading...</p>
        )}
        ...
        {!loading &&
          (tab === "created" ? published : games).map((game) => (
            <article
              key={game.id}
              className="overflow-hidden rounded-2xl border border-white/10 bg-[#17171a]"
            >
              <div
                  ...
                  background:
                    game.theme === "monster"
                      ...
                      : game.theme === "candy"
                        ...
                        : "linear-gradient(160deg,#334,#050508)",
                }}
              >
        ...
        {!loading && (tab === "created" ? published : games).length === 0 && tab !== "created" && (
          <p className="col-span-2 py-12 text-center text-sm text-white/40">Nothing here yet</p>
        )}
      </div>
    </div>
  );
